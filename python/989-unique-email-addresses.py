...
class Solution:
    ...
    Space Complexity: O(nm). In the worst case, when all emails are unique, we will store every email address given to
    us in the hash set.
    """
    ...
    def numUniqueEmails(self, emails: List[str]) -> int:
        ...
        for email in emails:
            ...
            username = "".join(username.split(".")).split("+")[0]
...
class Solution1:
    def numUniqueEmails(self, emails: List[str]) -> int:
        ...
        for email in emails:
            ...
            for letter in email:
                if letter == "+" and not is_domain:
                    is_plus = True
                elif letter == "@":
                    is_plus = False
                    is_domain = True
                    filtered_email += letter
                elif is_plus:
                    pass
                elif is_domain:
                    filtered_email += letter
