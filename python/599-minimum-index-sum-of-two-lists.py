...
class Solution:
    ...
    Space complexity : O(l1*x). Hashmap size grows up to l1*x, where x refers to average string length.
    """

    def findRestaurant(self, list1: List[str], list2: List[str]) -> List[str]:
        dict1 = {}
        dict2 = {}
        min_index_sum = 2000  # Max sum of two indexes of the arrays equals 2000
        ...
        for idx, s in enumerate(list2):
            if s in dict1:
                dict2[s] = idx + dict1[s]
                if min_index_sum > dict2[s]:
                    min_index_sum = dict2[s]

        result = []

        for s in dict2:
            if dict2[s] == min_index_sum:
                result.append(s)
