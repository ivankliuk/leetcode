...
class RecentCounter:
    ...
    def ping(self, t: int) -> int:
        """
        Time complexity O(1)  (amount elements to iterate is constant)
        Space complexity O(1) (queue length is constant)
        """
        self._queue.append(t)

        i = 0
        number_of_calls = len(self._queue)
        while i <= len(self._queue) - 1:
            if t >= self._queue[i] >= t - 3000:
                number_of_calls -= i
